 ...
 */
public class PreparedPacketsTest {
    ...
    public void testGetPacket() throws Exception {
        PreparedPackets preparedPackets1 = new PreparedPackets(buildCommand(new byte[0]));
        assertNotNull(preparedPackets1.getPacket(0));

        PreparedPackets preparedPackets2 = new PreparedPackets(buildCommand(UNPREPARED_PACKET));
        assertNotNull(preparedPackets2.getPacket(0));
        assertNotNull(preparedPackets2.getPacket(1));
        assertNotNull(preparedPackets2.getPacket(2));
    }

    @Test(expected = IndexOutOfBoundsException.class)
    public void testGetPacket1() throws Exception {
        PreparedPackets preparedPackets1 = new PreparedPackets(buildCommand(new byte[0]));
        preparedPackets1.getPacket(1);
    }

    @Test
    public void testSize() throws Exception {
        assertEquals(1, new PreparedPackets(buildCommand(new byte[0])).size());
        assertEquals(3, new PreparedPackets(buildCommand(UNPREPARED_PACKET)).size());
    }

    @Test
    public void testLength() throws Exception {
        // Length should be data length + 6 + (0x10 size * 2) for escaping
        assertEquals(6, new PreparedPackets(buildCommand(new byte[0])).length());
        assertEquals(UNPREPARED_PACKET.length + 7, new PreparedPackets(buildCommand(UNPREPARED_PACKET)).length());
    }

    @Test
    public void testPrepare() throws Exception {
        assertArrayEquals(new byte[]{0x10, 0x02, 0x00, 0x00, 0x10, 0x03}, PreparedPackets.prepare(buildCommand(new byte[0])));
        assertArrayEquals(COMPLETE_PACKET, PreparedPackets.prepare(buildCommand(UNPREPARED_PACKET)));
    }

    @Test
    public void testSplit() throws Exception {
        // Walk the source data and check each byte matches the split data
        List<byte[]> split = new ArrayList<>(PreparedPackets.split(COMPLETE_PACKET));

...
public class Crc {
     ...
     * <p/>
     * Based on CRC implementation use by app.
     *
     * @param input checksum input
     * @return CRC16 checksum
     */
    public static int crc16(byte[] input) {
        return crc16(input, 0, input.length);
    }

    /**
     * Perform CRC16 checksum on part of a byte array.
     * <p/>
     * Based on CRC implementation use by app.
     *
     * @param input  checksum input
     * @param start  the position of the first byte to check
     * @param length the number of bytes to check
     * @return CRC16 checksum
     */
    public static int crc16(byte[] input, int start, int length) {
        int crc = 0;
        for (int i = start; i < length; i++) {
            byte b = input[i];
            int left = crc << 8;
            int right = CRC_16_TABLE[(crc >> 8 ^ (char) b) & 0xff];
            crc = left ^ right;
        }
        return crc;
    }
